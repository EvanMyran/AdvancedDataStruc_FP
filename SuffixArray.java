...
// EDITED TO BETTER WORK WITH Sus.java
// EDITORS: Evan Myran, Andrew Martens
...
public class SuffixArray {
     ...
     * @param order determines if suffix is compared with previous or next entry
     * @return the length of the longest common prefix of the <em>i</em>th
     * smallest suffix and the <em>i</em>-1st smallest suffix.
     * @throws java.lang.IllegalArgumentException unless {@code 1 <= i < n}
     */
    public int lcp(int i, boolean order) {
        if (i < 1 || i >= suffixes.length) throw new IllegalArgumentException();
        if (order) return lcpSuffix(suffixes[i], suffixes[i-1]);
        return lcpSuffix(suffixes[i], suffixes[i+1]);
    }

    // longest common prefix of s and t
    // Returns 0 if there is no sus detected
    private static int lcpSuffix(Suffix s, Suffix t) {
        int n = Math.min(s.length(), t.length());
        for (int i = 0; i < n; i++) {
            if (s.charAt(i) != t.charAt(i)) return i + 1;
        }
        return 0;
    }

    // @param j an integer to stop search early
    public int lcp(int i, int j) {
        if (i < 1 || i >= suffixes.length) throw new IllegalArgumentException();
        return lcpSuffix(suffixes[i], suffixes[i-1], j);
    }

    // longest common prefix of s and t
    // Modified for finding sus.
    // param j is length of current shortest unique substring (sus)
    private static int lcpSuffix(Suffix s, Suffix t, int j) {
        int n = Math.min(s.length(), t.length());
        if(n > j) n = j;
        for (int i = 0; i < n; i++) {
            // If mismatch found
        ...
        return 0; // disregard prefix since it isn't unique
    }
